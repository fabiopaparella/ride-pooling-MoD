...
for i = 1:N_no
    ...
    IndNewNodes = idx(b);
    for j = IndNewNodes
        LinkLength1(i,j) = norm(NodesLoc(j,:) - NodesLoc(i,:)); %m
...
%saveas(figure(1),strcat('Images/','NYC_networks.pdf'))
NodesLoc = NodesLoc2;
save(strcat('Graphs',num2str(N_no),'.mat'), 'G_road','NodesLoc','LinkTime','FreeFlowTime','DemandS');
